...
export const SessionQuestionDock: Component<{ request: QuestionRequest; onSubmit: () => void }> = (props) => {
  ...
  const fold = () => setStore("collapsed", (v) => !v)
  ...
  return (
    ...
    >
      <div
        ...
        classList={{
          "mb-6": store.collapsed && (store.answers[store.tab]?.length ?? 0) === 0,
        }}
      ...
      <Show when={store.collapsed && (store.answers[store.tab]?.length ?? 0) > 0}>
        <div data-slot="question-hint" class="cursor-default mb-6">
          {store.answers[store.tab]?.length ?? 0} answer{(store.answers[store.tab]?.length ?? 0) === 1 ? "" : "s"}{" "}
          selected
        </div>
      </Show>
